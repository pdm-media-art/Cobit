function renderReport(){
  ...
  ac.forEach(ch=>ch.items.forEach(it=>{const f=S.findings[it.id],s=f?.status;if(s==='ok'){ok++;okItems.push({...it,sec:ch.l});}else if(s==='kritisch'){k.push({...it,note:f?.note||'',photos:f?.photos||[],sec:ch.l,icon:ch.i});const[a,b]=parseCost(it.c.kritisch);cMin+=a;cMax+=b;}else if(s==='mangel'){m.push({...it,note:f?.note||'',photos:f?.photos||[],sec:ch.l,icon:ch.i});const[a,b]=parseCost(it.c.mangel);cMin+=a;cMax+=b;}}));
  ...
  const fr=(f,p)=>`<div class="finding ${p}"><div class="finding-icon">${f.icon}</div><div class="f-body"><div class="f-label">${f.l} <span class="mi ${modCls(f.m)}">${modLabel(f.m)}</span>${f.m==='itgov'?` <span class="nb it" style="font-size:.42rem;margin:0">COBIT ${f.n}</span>`:''}</div><div class="f-section">${f.sec}${f.m!=='itgov'?' · '+f.n:''}</div>${f.note?`<div class="f-note">"${esc(f.note)}"</div>`:''}${f.photos&&f.photos.length?`<div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:6px">${f.photos.map(p=>`<img src="${p}" style="width:60px;height:46px;object-fit:cover;border-radius:5px;border:1px solid var(--border)">`).join('')}</div>`:''}</div><div class="f-cost"><div class="f-cost-val">€ ${f.c[p]}</div><div class="f-cost-lbl">${L==='en'?'Estimate':'Schätzung'}</div></div></div>`;
...
function _kskUmfeldHTML(){
  const L=typeof _LANG!=='undefined'?_LANG:'de';
  const addrStr=S.meta.strasse?[`${S.meta.strasse} ${S.meta.hausnummer||''}`.trim(),`${S.meta.plz||''} ${S.meta.ort||''}`.trim(),S.meta.land||''].filter(Boolean).join(', '):(S.meta.adresse||'');
  if(!S.umfeld?.done){
