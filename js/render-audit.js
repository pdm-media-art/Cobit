...
function renderUmfeld(){
  ...
  <div class="panel">
    ...
    <p class="panel-sub">${L==='en'?'Automated risk analysis of the surrounding area based on OpenStreetMap data. Select radius below.':'Automatische Risikoanalyse der Umgebung auf Basis von OpenStreetMap-Daten. Radius unten wählbar.'}</p>
    ...
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;flex-wrap:wrap">
      <label style="font-family:var(--fm);font-size:.52rem;letter-spacing:.08em;text-transform:uppercase;color:var(--soft);white-space:nowrap">${L==='en'?'Analysis Radius':'Analyse-Radius'}</label>
      <select id="uf-radius-sel" onchange="if(!S.umfeld)S.umfeld={};S.umfeld.selectedRadius=+this.value;save()" style="background:var(--surface2);border:1px solid var(--border);border-radius:6px;padding:5px 10px;color:var(--text2);font-size:.8rem;font-family:var(--ff);cursor:pointer;outline:none">
        ${[1000,5000,10000,15000].map(r=>`<option value="${r}" ${(u.selectedRadius||1000)===r?'selected':''}>${r>=1000?(r/1000)+' km':r+' m'}</option>`).join('')}
      </select>
    </div>
...
async function startUmfeldanalyse(){
  ...
  try{
    ...
    if(syn.length){score+=3;risks.push({level:'hoch',icon:'SYN',category:L==='en'?'Jewish Institution (Synagogue)':'Jüdische Einrichtung (Synagoge)',count:syn.length,desc:L==='en'?`${syn.length} synagogue(s) within ${radLbl}. High security requirement — frequently under police protection. Heightened vigilance against antisemitic violence.`:`${syn.length} Synagoge(n) im ${radLbl}-Radius. Erhöhter Schutzbedarf – häufig unter Polizeischutz. Besondere Wachsamkeit gegenüber antisemitischer Gewalt erforderlich.`,items:syn.map(p=>p.tags?.name||'Synagoge').slice(0,3),recommend:L==='en'?'Coordinate with local police authority. Review perimeter protection and CCTV coverage.':'Abstimmung mit zuständiger Polizeidienststelle empfohlen. Perimeterschutz und Videoüberwachung prüfen.'});}
    if(mos.length){score+=2;risks.push({level:'mittel',icon:'MOS',category:L==='en'?'Islamic Institution (Mosque)':'Islamische Einrichtung (Moschee)',count:mos.length,desc:L==='en'?`${mos.length} mosque(s) within ${radLbl}. Potential target for hate crime.`:`${mos.length} Moschee(n) im ${radLbl}-Radius. Mögliches Ziel für Hasskriminalität.`,items:mos.map(p=>p.tags?.name||'Moschee').slice(0,3),recommend:L==='en'?'CCTV at entrances, adequate lighting, police contact recommended.':'Videoüberwachung an Eingängen, gute Beleuchtung, Polizeikontakt empfohlen.'});}
    if(chr.length){score+=1;risks.push({level:'niedrig',icon:'CHR',category:L==='en'?'Christian Institution (Church)':'Christliche Einrichtung (Kirche)',count:chr.length,desc:L==='en'?`${chr.length} church(es) within ${radLbl}.`:`${chr.length} Kirche(n) im ${radLbl}-Radius.`,items:chr.map(p=>p.tags?.name||'Kirche').slice(0,3),recommend:L==='en'?'Standard security measures. Enhanced presence during major events.':'Standardsicherheitsmaßnahmen ausreichend. Bei Großveranstaltungen erhöhte Präsenz.'});}
    ...
    if(transit.length){score+=1;risks.push({level:'niedrig',icon:'TRN',category:L==='en'?'Public Transport Hub':'Öffentlicher Verkehrsknotenpunkt',count:transit.length,desc:L==='en'?`${transit.length} station(s) within ${radLbl}. High foot traffic.`:`${transit.length} Bahnhof/Haltestelle im ${radLbl}-Radius. Hoher Personenverkehr.`,items:transit.map(p=>p.tags?.name||'Haltestelle').slice(0,3),recommend:L==='en'?'Access control and CCTV at entrances especially important.':'Zutrittskontrolle und Videoüberwachung in Eingangsbereichen besonders wichtig.'});}
    if(banks.length){risks.push({level:'info',icon:'BNK',category:L==='en'?'Banks / Financial Institutions':'Banken / Finanzinstitute',count:banks.length,desc:L==='en'?`${banks.length} bank(s) within ${radLbl}.`:`${banks.length} Bank(en) im ${radLbl}-Radius.`,items:banks.map(p=>p.tags?.name||'Bank').slice(0,3),recommend:L==='en'?'Elevated risk for pickpocketing and cash transit in the area.':'Erhöhtes Risiko für Trickdiebstahl und Geldtransporte im Umfeld.'});}
    if(hosp.length){risks.push({level:'info',icon:'MED',category:L==='en'?'Hospitals / Medical Facilities':'Krankenhäuser / Gesundheitseinrichtungen',count:hosp.length,desc:L==='en'?`${hosp.length} hospital(s) within ${radLbl}.`:`${hosp.length} Krankenhaus/Klinik im ${radLbl}-Radius.`,items:hosp.map(p=>p.tags?.name||'Krankenhaus').slice(0,3),recommend:L==='en'?'Keep emergency access routes clear.':'Rettungszufahrten freihalten.'});}
    if(schools.length){risks.push({level:'info',icon:'EDU',category:L==='en'?'Schools / Educational Institutions':'Schulen / Bildungseinrichtungen',count:schools.length,desc:L==='en'?`${schools.length} school(s) within ${radLbl}. High pedestrian volume at peak times.`:`${schools.length} Schule(n) im ${radLbl}-Radius. Hoher Personenverkehr zu Stoßzeiten.`,items:schools.map(p=>p.tags?.name||'Schule').slice(0,3),recommend:L==='en'?'Account for peak hours in security planning.':'Stoßzeiten bei Sicherheitsplanung berücksichtigen.'});}
    if(pub.length){risks.push({level:'info',icon:'PUB',category:L==='en'?'Public Spaces / Landmarks':'Öffentliche Plätze / Sehenswürdigkeiten',count:pub.length,desc:L==='en'?`${pub.length} public venue(s) within ${radLbl}.`:`${pub.length} öffentliche Einrichtung(en) im ${radLbl}-Radius.`,items:pub.map(p=>p.tags?.name||'Einrichtung').slice(0,3),recommend:L==='en'?'Consider visitor flows and event calendars.':'Besucherströme und Veranstaltungskalender beachten.'});}
